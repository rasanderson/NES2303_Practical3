...
vegetation in one of your experimental plots. The result is that
...
that something very weird can start to happen. It can also arise if you have
one categorical and one continuous explanatory (ANCOVA).

### Recap: randomised block design
We'll begin by looking at some randomised block data. A randomised block is a
standard experimental setup, with two categorical explanatory variables. Note
that this problem of unbalanced designs can arise in any experimental
setup, and is not restricted to blocked designs. 

First, a balanced design. Here is a summary of the data:


```{r summary-balanced, echo=FALSE}
...
and you can see that there are three levels in the fertiliser categorical, and
four in the block categorical, with equal numbers in each level. This is nicely
balanced data. If you create two linear models, but swap the order of predictors
the the output results are, as you'd expect, identical:
...
In the second dataset, one replicate has been lost. This can easily happen in
both laboratory and field experiments. In the following dataset we've lost a
replicate:

```{r biomass_unbalanced, echo=FALSE, warning=FALSE}
plant_growth <- read.csv("www/plant_growth.csv")
plant_growth_lng <- pivot_longer(plant_growth,
                                 cols = Control:P,
                                 names_to = "fertiliser",
...
Notice how each level of our categorical predictors no long has the same number
of replicates. This is an unbalanced dataset. What happens if you swap the order
in your model of the predictors:

```{r biomass_lm_unbalanced-setup, echo=FALSE, warning=FALSE}
plant_growth <- read.csv("www/plant_growth.csv")
plant_growth_lng <- pivot_longer(plant_growth,
                                 cols = Control:P,
                                 names_to = "fertiliser",
