...
### Before you begin
1. Start RStudio in the usual way. Open your `NES2303` project from the main menu by clicking on **File -> Recent projects... -> NES2303**. Check that the word "None" has been replaced by "NES2303" in the project listing at the top right.
2. From the main menu click "File -> New file -> R Script"
3. Save your script, e.g. call it `practical3.R`
4. Write a comment line, beginning with a `#` symbol, e.g.
```{r eval =FALSE}
# Practical 3: investiage interaction terms
```
...
2. Using the File Explorer (main Windows menu) copy the `plasma.csv` file from `Downands` and paste into your `NES2303` project folder.
...
```{r plot-anrerr, warning=FALSE, echo=TRUE, eval=FALSE}
# plot data first
gf_point(d13c ~ standrad_length, col = ~area, data = anr) %>%
...
```{r plot-anr, warning=FALSE, echo=FALSE}
# plot data first
